...
class ODESystemNative(om.ExplicitComponent):
    ...
    def _pretrain(self,y,order,num_ctrl_pts,regularization_weight=1e-6):
        ...
        xlimits = np.array([[-0.0001, 46.0], [-0., 1.0001]]) 
    ...
    def _predict_eve(self, sm, xt_eve, xs_eve):
        ...
        print('x_eval_lower', x_eval_lower)
        print('x_eval_upper', x_eval_upper)
        print('x_eve', x_eve)
        print('self.xlimits', self.xlimits)

        dy_dt_soc_l = sm.predict_derivatives(x_eval_lower, 0).flatten()
        dsoc_dt_soc_l = sm.predict_derivatives(x_eval_lower, 1).flatten()
        b_sol_l = sm.predict_values(x_eval_lower)-(dy_dt_soc_l*x_eve[:,0] + dsoc_dt_soc_l*0)

        dy_dt_soc_u = sm.predict_derivatives(x_eval_upper, 0).flatten()
        dsoc_dt_soc_u = sm.predict_derivatives(x_eval_upper, 1).flatten()
        b_sol_u = sm.predict_values(x_eval_upper)-(dy_dt_soc_u*x_eve[:,0] + dsoc_dt_soc_u*1)



        y_predict = sm.predict_values(x_eve*interpolate_msk)*interpolate_msk +\
                     (dy_dt_soc_l * x_eve[:,0] + dsoc_dt_soc_l * x_eve[:,1] + b_sol_l)*lower_linear_msk +\
                        (dy_dt_soc_u * x_eve[:,0] + dsoc_dt_soc_u * x_eve[:,1] + b_sol_u)*upper_linear_msk
        ...
        dy_dt = (sm.predict_derivatives(x_eve*interpolate_msk, 0)*interpolate_msk).flatten() +\
                    dy_dt_soc_l*lower_linear_msk.flatten() + dy_dt_soc_u*upper_linear_msk.flatten()
        dy_dsoc =  (sm.predict_derivatives(x_eve*interpolate_msk, 1)*interpolate_msk).flatten() +\
                    dsoc_dt_soc_l*lower_linear_msk.flatten() + dsoc_dt_soc_u*upper_linear_msk.flatten()



        # print('y', sm.predict_values(x_eve*interpolate_msk)*interpolate_msk,
        #  (dy_dt_soc_l * x_eve[:,0] + dsoc_dt_soc_l * x_eve[:,1] + b_sol_l)*lower_linear_msk,
        #  (dy_dt_soc_u * x_eve[:,0] + dsoc_dt_soc_u * x_eve[:,1] + b_sol_u)*upper_linear_msk)
        # print('dy_dt', (sm.predict_derivatives(x_eve*interpolate_msk, 0)*interpolate_msk).flatten(),
        # dy_dt_soc_l*lower_linear_msk.flatten(), dy_dt_soc_u*upper_linear_msk.flatten())
        # print('dy_dsoc', (sm.predict_derivatives(x_eve*interpolate_msk, 1)*interpolate_msk).flatten(),
        # dsoc_dt_soc_l*lower_linear_msk.flatten() ,
        #  dsoc_dt_soc_u*upper_linear_msk.flatten())
        return y_predict, dy_dt, dy_dsoc
...
if __name__ == "__main__":
    ...
    prob.model.add_subsystem('ODESystemNative', ODESystemNative(n_s=200.))
    prob.setup()
    prob.set_val('ODESystemNative.SoC', -1.5)
    prob.set_val('ODESystemNative.T_cell', 20)
    prob.set_val('ODESystemNative.power_profile', 0.1)
